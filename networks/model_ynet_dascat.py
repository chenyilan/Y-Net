...
class YNet(nn.Module):
    ...
    def forward(self, x,bfimg):
         #encoder1: raw data
        x1, _ = self.down1(x) # 1280, 64,32
        x2, _ = self.down2(x1)# 640, 32,64
        x3, _ = self.down3(x2) # 320, 16,128
        x4, _ = self.down4(x3) # 160, 8,256
        x5 = self.bottom(x4)
         #encoder2: bf
        ...
        out = self.up1(bxbefore_pool4, out)# 16, 16,128
        out = self.up2(bxbefore_pool3, out)# 32, 32,64
        out = self.up3(bxbefore_pool2, out)# 64, 64,32
        out = self.up4(bxbefore_pool1, out)# 128, 128,1
        return out
