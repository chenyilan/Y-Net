...
class DownConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, pooling=True,raw_data=False):
...
class YNet(nn.Module):
    ...
    def __init__(self,  in_channels=3, up_mode='transpose', merge_mode='concat'):
        ...
        self.in_channels = in_channels
        self.rddown1 = DownConv(1,32,raw_data=True)
    ...
    def forward(self, x,bfimg):
         #encoder1: raw data
        x1, before_pool1 = self.rddown1(x) # 1280, 64,32
        x2, before_pool2 = self.rddown2(x1)# 640, 32,64
        x3, before_pool3 = self.rddown3(x2) # 320, 16,128
        x4, before_pool4 = self.rddown4(x3) # 160, 8,256
        x5 = self.bottom(x4)
         #encoder2: bf
        ...
        out = self.up1(before_pool4,bxbefore_pool4, out)# 16, 16,128
        out = self.up2(before_pool3,bxbefore_pool3, out)# 32, 32,64
        out = self.up3(before_pool2,bxbefore_pool2, out)# 64, 64,32
        out = self.up4(before_pool1,bxbefore_pool1, out)# 128, 128,1
        return out
